	...
	var dtPortfolioPostsWidgetHandler = function($scope, $){
		...
		function calculatePortfolioSingleItemHeight() {
			...
			if($('body').hasClass('single-dt_portfolios')) {
				...
				$('.fixed-gallery').find('.dtportfolio-details').height(fixed_height).niceScroll({zindex: 999999, cursorborder: '1px solid #424242' });

				$('.fixed-feature-image').find('.dtportfolio-fixed-content').height(height).css('top', headerHeight);
				$('.fixed-feature-image').find('.dtportfolio-details').height(fixed_height).niceScroll({zindex: 999999, cursorborder: '1px solid #424242' });
				
				$('.gallery-list').find('.dtportfolio-fixed-content').height(height).css('top', headerHeight);
				$('.gallery-list').find('.dtportfolio-details').height(fixed_height).niceScroll({zindex: 999999, cursorborder: '1px solid #424242' });
